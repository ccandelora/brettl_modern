...
export default class extends Controller {
  ...
  handleDragStart(event) {
    ...
    // Store drag data with a deep clone of the source element's data
    this.dragData = {
      ...
      assignableType:
        draggableElement.dataset.assignableType ||
        (draggableElement.textContent.includes('Guest') ? 'Guest' : 'Reservation'),
      assignableId: draggableElement.dataset.assignableId || draggableElement.dataset.reservationId,
      sourceElement: draggableElement,
      sourceHTML: draggableElement.outerHTML,
      sourceDropZone: draggableElement.closest('[data-bunk-editor-target="dropZone"]'),
      sourceUnassignedArea: draggableElement.closest('[data-bunk-editor-target="unassignedArea"]'),
    };
    ...
    event.dataTransfer.setData(
      'text/plain',
      JSON.stringify({
        reservationId: this.dragData.reservationId,
        reservationName: this.dragData.reservationName,
        reservationGender: this.dragData.reservationGender,
      })
    );
  ...
  handleDragEnter(event) {
    ...
    if (dropZone) {
      ...
      const assignedPerson = dropZone.querySelector(
        '[data-bunk-editor-target="assignedPerson"] .draggable-person'
      );
      if (assignedPerson && assignedPerson !== this.dragData.sourceElement) {
        // Bunk is already assigned to someone else - don't show drop feedback
        console.log('🚫 Bunk already assigned to someone else, no drop allowed');
        return;
      }
  ...
  handleDrop(event) {
    ...
    if (unassignedArea) {
      ...
      const sourceDropZone = this.dragData.sourceElement.closest(
    ...
    if (dropZone) {
      ...
      if (assignedPerson && assignedPerson !== this.dragData.sourceElement) {
        console.log('❌ Drop rejected: bunk already assigned to someone else');
        return;
      }
      ...
      if (this.dragData.sourceDropZone) {
        // Clear the source bunk if it's different from the target
        if (this.dragData.sourceDropZone !== dropZone) {
          const sourceBunkId = this.dragData.sourceDropZone.dataset.bunkId;
          const sourceSelect = this.element.querySelector(
            `select[name="assignments[${sourceBunkId}]"]`
          );
          if (sourceSelect) {
            sourceSelect.value = '';
          }
          const sourceAssignedArea = this.dragData.sourceDropZone.querySelector(
            '[data-bunk-editor-target="assignedPerson"]'
          );
          sourceAssignedArea.innerHTML = `
      ...
      } else if (this.dragData.sourceUnassignedArea) {
        // Remove from unassigned area
        this.dragData.sourceElement.remove();
      }
  ...
  handleSelectChange(event) {
    ...
    if (reservationId) {
      ...
      // Remove from unassigned area only
      this.removeFromSource(reservation.id, null);
    ...
    if (select) {
      ...
      if (option) {
        ...
        const fallbackOption = Array.from(select.options).find(
          (opt) => opt.value && opt.value.includes(reservationId.toString())
        );
        if (fallbackOption) {
  ...
  removeFromSource(reservationId, sourceElement = null) {
    if (sourceElement) {
      // Only remove from the specific source location
      const sourceDropZone = sourceElement.closest('[data-bunk-editor-target="dropZone"]');
      if (sourceDropZone) {
        const assignedPersonArea = sourceDropZone.querySelector(
          '[data-bunk-editor-target="assignedPerson"]'
        );
        if (assignedPersonArea.contains(sourceElement)) {
          assignedPersonArea.innerHTML = `
            <div class="text-center py-3 text-gray-400 text-sm border-2 border-dashed border-gray-200 rounded-lg">
              Drop here or select below
            </div>
        ...
        // If coming from unassigned area, just remove the element
        sourceElement.remove();
      }
    } else {
      // If no source element provided, remove from unassigned areas only
      const unassignedElements = this.element.querySelectorAll(
        `[data-bunk-editor-target="unassignedArea"] .draggable-person[data-reservation-id="${reservationId}"]`
      );
  ...
  moveToUnassigned(reservationData) {
    ...
    if (gender === 'female') {
      targetArea = this.element.querySelector(
        '[data-gender="women"][data-bunk-editor-target="unassignedArea"]'
      );
    } else if (gender === 'male') {
      targetArea = this.element.querySelector(
        '[data-gender="men"][data-bunk-editor-target="unassignedArea"]'
      );
    } else {
      // For unknown gender, add to coed area
      targetArea = this.element.querySelector(
        '[data-gender="coed"][data-bunk-editor-target="unassignedArea"]'
      );
    }
  ...
  findDropZoneByBunkId(bunkId) {
    return this.element.querySelector(
      `[data-bunk-editor-target="dropZone"][data-bunk-id="${bunkId}"]`
    );
  }
  ...
  guessGenderFromDropdown(option) {
    ...
    if (select) {
      ...
      if (dropZone) {
        return dropZone.dataset.gender === 'women'
          ? 'female'
          : dropZone.dataset.gender === 'men'
          ? 'male'
          : 'unknown';
      }
    }
  ...
  quickAssign(event) {
    ...
    const availableBunks = Array.from(this.dropZoneTargets).filter((zone) => {
      ...
      return (
        this.isGenderCompatible(zoneGender, reservationGender) &&
  ...
  isGuestId(id) {
    ...
    const guestElement = this.element.querySelector(
      `[data-assignable-type="Guest"][data-assignable-id="${id}"]`
    );
    return !!guestElement;
  }
}
